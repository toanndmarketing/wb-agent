/**
 * Module: Rank Comparison 5 Days (5-day comparison)
 * So sánh hiệu suất từ khóa & rank của 5 ngày gần nhất có dữ liệu (Period A) so với 5 ngày trước đó (Period B).
 * Giúp phát hiện sự thay đổi thứ hạng ổn định hơn và tìm các từ khóa mới nổi có volume cao.
 */
...
async function run(config) {
    ...
    printHeader('Rank & Traffic Comparison (5-Day Periods)', config);
    ...
    // 2. Tính toán 2 chu kỳ 5 ngày
    // Period A (Current 5 days): [latestDate - 4 days, latestDate]
    // Period B (Previous 5 days): [latestDate - 9 days, latestDate - 5 days]
    const latestDate = new Date(latestDateStr);
    
    const dateAEnd = latestDateStr;
    const dateAStart = new Date(latestDate.getTime() - 4 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];

    const dateBEnd = new Date(latestDate.getTime() - 5 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    const dateBStart = new Date(latestDate.getTime() - 9 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];

    console.log(`  -> Chu kỳ A (5 ngày hiện tại): ${dateAStart} đến ${dateAEnd}`);
    console.log(`  -> Chu kỳ B (5 ngày trước đó): ${dateBStart} đến ${dateBEnd}`);
    ...
    // Chỉ tính các query có Impressions > 15 ở chu kỳ A để tránh rank ảo
    const rankGainers = comparisonList
        .filter(item => !item.isLost && item.rankDiff > 0.5 && item.currImpressions > 15)
        .sort((a, b) => b.rankDiff - a.rankDiff)
        .slice(0, 20);

    // 4.2 Giảm hạng nhiều nhất (Rank Losers)
    const rankLosers = comparisonList
        .filter(item => item.rankDiff < -0.5 && (item.prevImpressions > 15 || item.currImpressions > 15))
    ...
    // 4.5 Từ khóa MỚI NỔI có Volume cao (New High-Volume Keywords)
    // Định nghĩa: Chu kỳ trước có rất ít hoặc không có hiển thị (prevImpressions <= 5),
    // Chu kỳ hiện tại hiển thị tăng vọt (currImpressions >= 25)
    const newHighVolume = comparisonList
        .filter(item => item.prevImpressionsRaw <= 5 && item.currImpressions >= 20)
    ...
    const files = getReportFilename('rank-comparison-5days', domain);
    await saveJsonReport(files.json, reportData);

    // 6. Xây dựng Báo cáo Markdown (Vietnamese)
    let md = `# 📈 Báo Cáo So Sánh Rank & Traffic 5 Ngày — ${domain}\n\n`;
    md += `## 📊 Tóm Tắt Hiệu Suất Tổng Quan\n`;
    md += `- **Chu kỳ hiện tại (5 ngày A):** ${dateAStart} đến ${dateAEnd}\n`;
    md += `- **Chu kỳ đối chiếu (5 ngày B trước đó):** ${dateBStart} đến ${dateBEnd}\n\n`;
    ...
    // 6.1 Từ khóa MỚI NỔI có Volume cao (Crucial for User's Question)
    md += `## 🆕 1. Từ Khóa Mới Xuất Hiện Có Volume (Hiển Thị) Cao\n`;
    md += `> Các từ khóa trước đó hầu như không có hiển thị (impressions <= 5) nhưng hiện tại đã bứt phá đạt lượt hiển thị lớn. Đây là cơ hội tối ưu nội dung để hốt trọn traffic.\n\n`;
    md += mdTable(
        ['Từ Khóa', 'Trang Đích', 'Hiển Thị 5 Ngày A', 'Clicks 5 Ngày A', 'Rank 5 Ngày A', 'Hiển Thị Trước'],
    ...
    md += `> Top các từ khóa tăng hạng mạnh nhất (trung bình vị trí được cải thiện nhiều nhất trong 5 ngày).\n\n`;
    md += mdTable(
